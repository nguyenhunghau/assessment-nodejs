...
export async function register(input: {
  ...
  password: string;
  role?: UserRole;
}): Promise<{ user: Pick<User, 'id' | 'email' | 'role'>; token: string }> {
  ...
  // Only allow employee role for public registration
  if (input.role && input.role !== 'employee') {
    logger.warn('Registration failed: Admin role not allowed', { email });
    throw new Error('Admin role registration is not allowed');
  }
