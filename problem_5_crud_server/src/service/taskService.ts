...
export async function createTask(
  ...
  logger.debug('Creating task', {
    title: data.title,
    assignedTo: data.assigned_to_user_id,
  ...
  if (data.status && !validStatuses.includes(data.status)) {
    logger.warn('Task creation failed: Invalid status', {
      status: data.status,
    });
  ...
  if (data.priority && !validPriorities.includes(data.priority)) {
    logger.warn('Task creation failed: Invalid priority', {
  ...
  if (!data.assigned_to_user_id || data.assigned_to_user_id <= 0) {
    logger.warn('Task creation failed: Invalid assigned_to_user_id', {
      assignedTo: data.assigned_to_user_id,
    });
  ...
  const assignedUserExists = await userQueries.userExistsById(
    data.assigned_to_user_id
  );
  if (!assignedUserExists) {
    logger.warn('Task creation failed: Assigned user does not exist', {
      assignedTo: data.assigned_to_user_id,
    });
  ...
  if (!data.created_by_user_id || data.created_by_user_id <= 0) {
    logger.warn('Task creation failed: Invalid created_by_user_id', {
      createdBy: data.created_by_user_id,
    });
...
export async function updateTask(
  ...
  if (updates.status) {
    ...
    if (!validStatuses.includes(updates.status)) {
      logger.warn('Task update failed: Invalid status', {
        taskId: id,
  ...
  if (updates.priority) {
    ...
    if (!validPriorities.includes(updates.priority)) {
      logger.warn('Task update failed: Invalid priority', {
        taskId: id,
  ...
  if (updates.assigned_to_user_id !== undefined) {
    const userExists = await userQueries.userExistsById(
      updates.assigned_to_user_id
    );
    if (!userExists) {
      logger.warn('Task update failed: Assigned user does not exist', {
        taskId: id,
        assignedTo: updates.assigned_to_user_id,
      });
      throw new Error('Assigned user does not exist');
    }
  }
