...
export async function createEmployee(
  ...
  if (!data.user_id || data.user_id <= 0) {
    logger.warn('Employee creation failed: Invalid user_id', {
      userId: data.user_id,
    });
  ...
  if (!userExists) {
    logger.warn('Employee creation failed: User does not exist', {
      userId: data.user_id,
    });
    throw new Error('User does not exist');
  }

  // Check if user_id is already assigned to another employee
  const employeeWithUserId = await employeeQueries.employeeExistsByUserId(
    data.user_id
  );
  if (employeeWithUserId) {
    logger.warn(
      'Employee creation failed: user_id already assigned to another employee',
      { userId: data.user_id }
    );
    throw new Error('user_id is already assigned to another employee');
  }

  if (!data.first_name || !data.last_name) {
    logger.warn('Employee creation failed: Missing name', {
      userId: data.user_id,
    });
...
export async function updateEmployee(
  ...
  if (updates.user_id !== undefined) {
    ...
    if (!userExists) {
      logger.warn('Employee update failed: User does not exist', {
        employeeId: id,
    ...
    // Check if user_id is already assigned to another employee
    const existingEmployee = await employeeQueries.getEmployeeByUserId(
      updates.user_id
    );
    if (existingEmployee && existingEmployee.id !== id) {
      logger.warn(
        'Employee update failed: user_id already assigned to another employee',
        {
          employeeId: id,
          userId: updates.user_id,
          conflictingEmployeeId: existingEmployee.id,
        }
      );
      throw new Error('user_id is already assigned to another employee');
    }
