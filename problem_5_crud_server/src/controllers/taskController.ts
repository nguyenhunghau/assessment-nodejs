...
export const TaskController = {
  create: async (req: Request, res: Response) => {
    try {
      ...
      logger.info('Create task request successful', {
        taskId: task.id,
        createdBy: req.user.id,
      });
      ...
      logger.warn('Create task request failed', {
  ...
  list: async (req: Request, res: Response) => {
    try {
      ...
      logger.info('List tasks request successful', {
        userId: req.user.id,
  ...
  getById: async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        task.created_by_user_id !== req.user.id
      ) {
        logger.warn('Get task by ID failed: Forbidden', {
          taskId: id,
          userId: req.user.id,
        });
      ...
      logger.info('Get task by ID request successful', {
        taskId: id,
        userId: req.user.id,
      });
      return res.status(200).json({
      ...
      logger.warn('Get task by ID request failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      const statusCode =
  ...
  update: async (req: Request, res: Response) => {
    try {
      ...
      if (
        existing.assigned_to_user_id !== req.user.id &&
        existing.created_by_user_id !== req.user.id
      ) {
        logger.warn('Update task failed: Forbidden', {
          taskId: id,
          userId: req.user.id,
        });
      ...
      logger.info('Update task request successful', {
        taskId: id,
        userId: req.user.id,
      });
      return res.status(200).json({
      ...
      logger.warn('Update task request failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      const statusCode =
        error instanceof Error && error.message === 'Task not found'
  ...
  remove: async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        logger.warn('Delete task failed: Forbidden', {
          taskId: id,
          userId: req.user.id,
        });
        return res.status(403).json({ success: false, message: 'Forbidden' });
      ...
      logger.info('Delete task request successful', {
        taskId: id,
        userId: req.user.id,
      });
      return res.status(200).json({
      ...
      logger.warn('Delete task request failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      const statusCode =
        error instanceof Error && error.message === 'Task not found'
